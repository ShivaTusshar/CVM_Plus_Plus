...
```bash
./cvmpp                              # To run the interactive REPL
```

```bash
./cvmpp fizzbuzz.cvm        # To run a script file directly
```

```bash
./cvmpp --debug factorial.cvm   # To compile a script, view the generated bytecode, and then execute it
```
...
| `truth_machine.cvm` | `input`, infinite loop, `if/else`                |
| `calculator.cvm`    | multiple `input`s, nested `if/else`, arithmetic  |

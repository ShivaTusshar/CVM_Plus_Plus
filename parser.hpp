#pragma once
#include "ast.hpp"
#include "token.hpp"
#include <vector>
#include <stdexcept>
#include <initializer_list>

namespace cvm {

// Grammar (lowest -> highest precedence):
//
//   program     -> declaration* EOF
//   declaration -> fnDecl | letDecl | statement
//   fnDecl      -> "fn" IDENT "(" paramList? ")" block
//   paramList   -> IDENT ("," IDENT)*
//   letDecl     -> "let" IDENT "=" expression ";"
//
//   statement   -> printStmt | ifStmt | whileStmt | forStmt
//                | returnStmt | breakStmt | continueStmt | block | exprStmt
//
//   block       -> "{" declaration* "}"
//   forStmt     -> "for" "(" (letDecl | exprStmt | ";")
//                          expression? ";" expression? ")" statement
//   returnStmt  -> "return" expression? ";"
//
//   expression  -> assignment
//   assignment  -> IDENT "=" assignment | logicOr
//   logicOr     -> logicAnd ( "||" logicAnd )*
//   logicAnd    -> equality ( "&&" equality )*
//   equality    -> comparison ( "==" comparison )*
//   comparison  -> term ( "<" term )*
//   term        -> factor ( ("+"|"-") factor )*
//   factor      -> unary ( ("*"|"/") unary )*
//   unary       -> ("-"|"!") unary | call
//   call        -> primary ( "(" args? ")" )*
//   args        -> expression ( "," expression )*
//   primary     -> NUMBER | "true" | "false" | "nil" | "input"
//                | IDENT | "(" expression ")"
class Parser {
public:
    explicit Parser(std::vector<Token> toks) : tokens(std::move(toks)) {}

    std::vector<StmtPtr> parse() {
        std::vector<StmtPtr> stmts;
        while (!isAtEnd()) stmts.push_back(declaration());
        return stmts;
    }

private:
    std::vector<Token> tokens;
    size_t pos = 0;

    const Token& peek() const     { return tokens[pos]; }
    const Token& previous() const { return tokens[pos - 1]; }
    bool isAtEnd() const          { return peek().type == TokenType::EOF_TOK; }
    bool check(TokenType t) const { return !isAtEnd() && peek().type == t; }
    const Token& advance()        { if (!isAtEnd()) ++pos; return previous(); }
    bool match(std::initializer_list<TokenType> ts) {
        for (auto t : ts) if (check(t)) { advance(); return true; }
        return false;
    }
    const Token& consume(TokenType t, const std::string& msg) {
        if (check(t)) return advance();
        throw std::runtime_error("[line " + std::to_string(peek().line) +
                                 "] Parse error: " + msg + " (got '" + peek().lexeme + "')");
    }

    // ---- declarations ----
    StmtPtr declaration() {
        if (match({TokenType::FN}))  return fnDecl();
        if (match({TokenType::LET})) return letDecl();
        return statement();
    }

    StmtPtr fnDecl() {
        int ln = previous().line;
        const Token& nm = consume(TokenType::IDENTIFIER, "Expected function name after 'fn'.");
        consume(TokenType::LPAREN, "Expected '(' after function name.");
        std::vector<std::string> params;
        if (!check(TokenType::RPAREN)) {
            do {
                if (params.size() >= 32)
                    throw std::runtime_error("[line " + std::to_string(peek().line) +
                                             "] Too many parameters (max 32).");
                const Token& p = consume(TokenType::IDENTIFIER, "Expected parameter name.");
                params.push_back(p.lexeme);
            } while (match({TokenType::COMMA}));
        }
        consume(TokenType::RPAREN, "Expected ')' after parameters.");
        consume(TokenType::LBRACE, "Expected '{' before function body.");
        StmtPtr body = block();   // block consumes the closing '}'
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::FnDecl; s->name = nm.lexeme;
        s->params = std::move(params); s->body = std::move(body); s->line = ln;
        return s;
    }

    StmtPtr letDecl() {
        int ln = previous().line;
        const Token& nm = consume(TokenType::IDENTIFIER, "Expected variable name after 'let'.");
        consume(TokenType::EQUAL, "Expected '=' after variable name.");
        ExprPtr init = expression();
        consume(TokenType::SEMICOLON, "Expected ';' after variable declaration.");
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::VarDecl; s->name = nm.lexeme;
        s->expr = std::move(init); s->line = ln;
        return s;
    }

    // ---- statements ----
    StmtPtr statement() {
        if (match({TokenType::PRINT}))    return printStmt();
        if (match({TokenType::IF}))       return ifStmt();
        if (match({TokenType::WHILE}))    return whileStmt();
        if (match({TokenType::FOR}))      return forStmt();
        if (match({TokenType::RETURN}))   return returnStmt();
        if (match({TokenType::BREAK}))    return breakStmt();
        if (match({TokenType::CONTINUE})) return continueStmt();
        if (match({TokenType::LBRACE}))   return block();
        return exprStmt();
    }

    StmtPtr printStmt() {
        int ln = previous().line;
        ExprPtr v = expression();
        consume(TokenType::SEMICOLON, "Expected ';' after print value.");
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::Print; s->expr = std::move(v); s->line = ln; return s;
    }

    StmtPtr ifStmt() {
        int ln = previous().line;
        consume(TokenType::LPAREN, "Expected '(' after 'if'.");
        ExprPtr cond = expression();
        consume(TokenType::RPAREN, "Expected ')' after if condition.");
        StmtPtr thenB = statement();
        StmtPtr elseB = nullptr;
        if (match({TokenType::ELSE})) elseB = statement();
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::If; s->expr = std::move(cond);
        s->thenBranch = std::move(thenB); s->elseBranch = std::move(elseB); s->line = ln;
        return s;
    }

    StmtPtr whileStmt() {
        int ln = previous().line;
        consume(TokenType::LPAREN, "Expected '(' after 'while'.");
        ExprPtr cond = expression();
        consume(TokenType::RPAREN, "Expected ')' after while condition.");
        StmtPtr body = statement();
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::While; s->expr = std::move(cond);
        s->body = std::move(body); s->line = ln;
        return s;
    }

    StmtPtr forStmt() {
        int ln = previous().line;
        consume(TokenType::LPAREN, "Expected '(' after 'for'.");

        // init: let-decl, expr-stmt, or empty (just ';')
        StmtPtr init = nullptr;
        if (match({TokenType::SEMICOLON})) {
            init = nullptr;
        } else if (match({TokenType::LET})) {
            init = letDecl();   // consumes its ';'
        } else {
            init = exprStmt();  // consumes its ';'
        }

        // condition: optional expression, then ';'
        ExprPtr cond = nullptr;
        if (!check(TokenType::SEMICOLON)) cond = expression();
        consume(TokenType::SEMICOLON, "Expected ';' after for condition.");

        // step: optional expression, then ')'
        ExprPtr step = nullptr;
        if (!check(TokenType::RPAREN)) step = expression();
        consume(TokenType::RPAREN, "Expected ')' after for clauses.");

        StmtPtr body = statement();

        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::For; s->line = ln;
        s->forInit = std::move(init);
        s->expr    = std::move(cond);
        s->forStep = std::move(step);
        s->body    = std::move(body);
        return s;
    }

    StmtPtr returnStmt() {
        int ln = previous().line;
        ExprPtr v = nullptr;
        if (!check(TokenType::SEMICOLON)) v = expression();
        consume(TokenType::SEMICOLON, "Expected ';' after return.");
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::Return; s->expr = std::move(v); s->line = ln;
        return s;
    }

    StmtPtr breakStmt() {
        int ln = previous().line;
        consume(TokenType::SEMICOLON, "Expected ';' after 'break'.");
        auto s = std::make_unique<Stmt>(); s->kind = StmtKind::Break; s->line = ln; return s;
    }

    StmtPtr continueStmt() {
        int ln = previous().line;
        consume(TokenType::SEMICOLON, "Expected ';' after 'continue'.");
        auto s = std::make_unique<Stmt>(); s->kind = StmtKind::Continue; s->line = ln; return s;
    }

    StmtPtr block() {
        int ln = previous().line;
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::Block; s->line = ln;
        while (!check(TokenType::RBRACE) && !isAtEnd())
            s->statements.push_back(declaration());
        consume(TokenType::RBRACE, "Expected '}' to close block.");
        return s;
    }

    StmtPtr exprStmt() {
        int ln = peek().line;
        ExprPtr e = expression();
        consume(TokenType::SEMICOLON, "Expected ';' after expression.");
        auto s = std::make_unique<Stmt>();
        s->kind = StmtKind::Expression; s->expr = std::move(e); s->line = ln; return s;
    }

    // ---- expressions ----
    ExprPtr expression() { return assignment(); }

    ExprPtr assignment() {
        ExprPtr expr = logicOr();
        if (match({TokenType::EQUAL})) {
            int ln = previous().line;
            ExprPtr value = assignment();
            if (expr->kind != ExprKind::Variable)
                throw std::runtime_error("[line " + std::to_string(ln) + "] Invalid assignment target.");
            return makeAssign(expr->name, std::move(value), ln);
        }
        return expr;
    }

    ExprPtr logicOr() {
        ExprPtr expr = logicAnd();
        while (match({TokenType::PIPE_PIPE})) {
            int ln = previous().line;
            ExprPtr right = logicAnd();
            expr = makeLogical(TokenType::PIPE_PIPE, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr logicAnd() {
        ExprPtr expr = equality();
        while (match({TokenType::AMP_AMP})) {
            int ln = previous().line;
            ExprPtr right = equality();
            expr = makeLogical(TokenType::AMP_AMP, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr equality() {
        ExprPtr expr = comparison();
        while (match({TokenType::EQUAL_EQUAL})) {
            int ln = previous().line;
            ExprPtr right = comparison();
            expr = makeBinary(TokenType::EQUAL_EQUAL, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr comparison() {
        ExprPtr expr = term();
        while (match({TokenType::LESS})) {
            int ln = previous().line;
            ExprPtr right = term();
            expr = makeBinary(TokenType::LESS, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr term() {
        ExprPtr expr = factor();
        while (match({TokenType::PLUS, TokenType::MINUS})) {
            TokenType op = previous().type;
            int ln = previous().line;
            ExprPtr right = factor();
            expr = makeBinary(op, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr factor() {
        ExprPtr expr = unary();
        while (match({TokenType::STAR, TokenType::SLASH})) {
            TokenType op = previous().type;
            int ln = previous().line;
            ExprPtr right = unary();
            expr = makeBinary(op, std::move(expr), std::move(right), ln);
        }
        return expr;
    }

    ExprPtr unary() {
        if (match({TokenType::MINUS, TokenType::BANG})) {
            TokenType op = previous().type;
            int ln = previous().line;
            ExprPtr right = unary();
            return makeUnary(op, std::move(right), ln);
        }
        return call();
    }

    ExprPtr call() {
        ExprPtr expr = primary();
        while (match({TokenType::LPAREN})) {
            int ln = previous().line;
            std::vector<ExprPtr> args;
            if (!check(TokenType::RPAREN)) {
                do {
                    if (args.size() >= 32)
                        throw std::runtime_error("[line " + std::to_string(peek().line) +
                                                 "] Too many arguments (max 32).");
                    args.push_back(expression());
                } while (match({TokenType::COMMA}));
            }
            consume(TokenType::RPAREN, "Expected ')' after arguments.");
            expr = makeCall(std::move(expr), std::move(args), ln);
        }
        return expr;
    }

    ExprPtr primary() {
        if (match({TokenType::NUMBER})) {
            int64_t v = std::stoll(previous().lexeme);
            return makeNumber(v, previous().line);
        }
        if (match({TokenType::TRUE}))  return makeBool(true,  previous().line);
        if (match({TokenType::FALSE})) return makeBool(false, previous().line);
        if (match({TokenType::NIL}))   return makeNil(previous().line);
        if (match({TokenType::INPUT})) return makeInput(previous().line);
        if (match({TokenType::IDENTIFIER}))
            return makeVariable(previous().lexeme, previous().line);
        if (match({TokenType::LPAREN})) {
            ExprPtr e = expression();
            consume(TokenType::RPAREN, "Expected ')' after expression.");
            return e;
        }
        throw std::runtime_error("[line " + std::to_string(peek().line) +
                                 "] Expected expression, got '" + peek().lexeme + "'.");
    }
};

} // namespace cvm

...
int runSource(const std::string& source) {
    ...
    try {
        ...
        Compiler compiler;
        Function* main = compiler.compile(program);
        ...
        VM vm;
        // Compiler must outlive VM::run() — Function pointers reference it.
        return vm.run(main) == VM::Result::OK ? 0 : 70;
